...
weight_path = os.path.join(config["weight_path"],config["dataset"],config["model"]) 
filename = config["model"]
log_path = os.path.join(config["experiment_data_path"],config["dataset"],config["model"]) 
...
train_transform = transforms.Compose(
    ...
    # transforms.autoaugment.TrivialAugmentWide(), #distable for resnet
    transforms.ToTensor(),
    ...
    with tqdm(total=len(dataloader)) as pbar:
        with torch.no_grad():
            ...
            if accuracy > best_acc:
                best_acc = accuracy
                if save:
                    PATH = os.path.join(weight_path,f"Model@{config['model']}_ACC@{best_acc}.pt")
...
def train(epoch,network,optimizer,dataloader):
    ...
    with tqdm(total=len(dataloader)) as pbar:
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            outputs = network(inputs)
            loss = criterion(outputs, labels)
            mse_loss = net.get_mse_loss()

            normalized_mse_loss = mse_loss.sum() / mse_loss.std()

            loss += (0.1 * normalized_mse_loss)
